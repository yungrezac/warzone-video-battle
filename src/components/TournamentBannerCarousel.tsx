...
const TournamentBannerCarousel: React.FC = () => {
  const {
    data: banners,
    isLoading
        ...
        <div className="relative h-48 overflow-hidden">
          {activeBanners.map((banner, index) => <div key={banner.id} className={`absolute inset-0 transition-transform duration-500 ease-in-out ${index === currentIndex ? 'translate-x-0' : index < currentIndex ? '-translate-x-full' : 'translate-x-full'}`}>
              ...
              e.currentTarget.style.display = 'none';
                ...
                <div className="absolute inset-0 bg-black/20 flex items-end py-0 px-0">
          ...
          {activeBanners.length > 1 && <>
              <Button variant="ghost" size="sm" className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white border-0" onClick={prevBanner}>
                <ChevronLeft className="w-4 h-4" />
              </Button>
              <Button variant="ghost" size="sm" className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white border-0" onClick={nextBanner}>
...
export default TournamentBannerCarousel;