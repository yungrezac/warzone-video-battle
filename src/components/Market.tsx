...
const Market: React.FC = () => {
  ...
  const { data: userItems, isLoading: isLoadingUserItems } = useUserMarketItems();
  ...
  const userCategories = [
    { id: 'all', name: 'Все товары', icon: ShoppingCart },
    { id: 'защита', name: 'Защита', icon: Gift },
    { id: 'колеса', name: 'Колеса', icon: Star },
    { id: 'подшипники', name: 'Подшипники', icon: Gift },
    { id: 'одежда', name: 'Одежда', icon: Star },
    { id: 'аксессуары', name: 'Аксессуары', icon: Gift },
    { id: 'запчасти', name: 'Запчасти', icon: Star },
    { id: 'другое', name: 'Другое', icon: Gift },
  ];
          ...
          <TabsContent value="rubles">
            ...
            <Tabs value={selectedCategory} onValueChange={setSelectedCategory}>
              ...
              {userCategories.map(category => (
                <TabsContent key={category.id} value={category.id} className="mt-3">
                  {filteredUserItems && filteredUserItems.length > 0 ? (
                    <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">
                        {selectedCategory === 'all' 
                          ? 'Пользовательские товары скоро появятся' 
