...
const TelegramIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <Card className={`bg-gradient-to-r ${currentBanner.gradient} text-white border-0 shadow-lg transform transition-all duration-500 hover:scale-[1.02] h-[100px] relative overflow-hidden rounded-xl`}>
        ...
        <div className="flex items-center h-full p-4 relative z-10">
          ...
          <div className="flex-shrink-0">
            <Button onClick={() => handleBannerClick(currentBanner.url)} variant="secondary" size="sm" className="bg-white/20 hover:bg-white/30 text-white border-white/30 flex items-center gap-2 h-8 text-sm px-3 backdrop-blur-sm transition-all duration-200 hover:scale-105">
              <TelegramIcon />
              Присоединиться
        ...
        <div className="absolute bottom-2 right-3 flex gap-1">
          {chatBanners.map((_, index) => <div key={index} className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-white' : 'bg-white/40'}`} />)}
        </div>
      </Card>
    </div>;
};
