...
const TopUsers: React.FC = () => {
  ...
  return <div className="pb-16">
      ...
      {/* Top 3 Podium */}
      {topUsers && topUsers.length >= 3 && <div className="p-4 bg-gray-50">
          <div className="flex justify-center items-end space-x-2 mb-4">
            ...
            <div className="flex flex-col items-center">
              <div className="bg-gradient-to-t from-gray-300 to-gray-500 text-white rounded-lg p-3 mb-2 shadow-lg">
                <img src={topUsers[1]?.user?.avatar_url || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=60&h=60&fit=crop&crop=face'} alt={topUsers[1]?.user?.username || 'Роллер'} className="w-12 h-12 rounded-full border-2 border-white" />
              </div>
            ...
            <div className="flex flex-col items-center">
              <div className="bg-gradient-to-t from-yellow-400 to-yellow-600 text-white rounded-lg p-4 mb-2 shadow-xl transform scale-110">
                <img src={topUsers[0]?.user?.avatar_url || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=70&h=70&fit=crop&crop=face'} alt={topUsers[0]?.user?.username || 'Роллер'} className="w-14 h-14 rounded-full border-2 border-white" />
            ...
            <div className="flex flex-col items-center">
              <div className="bg-gradient-to-t from-amber-400 to-amber-600 text-white rounded-lg p-3 mb-2 shadow-lg">
                <img src={topUsers[2]?.user?.avatar_url || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=60&h=60&fit=crop&crop=face'} alt={topUsers[2]?.user?.username || 'Роллер'} className="w-12 h-12 rounded-full border-2 border-white" />
              </div>
              <div className="text-center">
                <div className="text-sm font-bold text-gray-700 flex items-center justify-center gap-1">
        ...
        <div className="bg-white rounded-lg shadow-md">
          ...
          <div className="divide-y divide-gray-100">
            {topUsers?.map((userPoint, index) => <div key={userPoint.id} className="p-3 flex items-center space-x-3 hover:bg-gray-50 transition-colors">
                ...
                <img src={userPoint.user?.avatar_url || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=50&h=50&fit=crop&crop=face'} alt={userPoint.user?.username || 'Роллер'} className="w-10 h-10 rounded-full border border-gray-200" />
                ...
                <div className="flex flex-col items-end">
                  ...
                  {userPoint.wins_count > 0 && <div className="text-xs text-yellow-600 font-medium">
